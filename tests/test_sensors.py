...
def test_collect_readings_prefers_lm_sensors(monkeypatch) -> None:
    ...
    monkeypatch.setattr("delltemp.sensors._from_psutil_temps", lambda: psutil_temp)
    monkeypatch.setattr("delltemp.sensors._from_psutil_fans", lambda: [])
    monkeypatch.setattr("delltemp.sensors._from_lm_sensors", lambda: lm_temp)
    monkeypatch.setattr("delltemp.sensors._from_nvidia_smi", lambda: [])
...
def test_collect_readings_isolates_collector_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("delltemp.sensors._from_psutil_temps", boom)
    monkeypatch.setattr("delltemp.sensors._from_psutil_fans", boom)
    monkeypatch.setattr("delltemp.sensors._from_lm_sensors", boom)
    monkeypatch.setattr("delltemp.sensors._from_nvidia_smi", lambda: nvidia)
    readings = collect_readings()
    assert len(readings) == 1
