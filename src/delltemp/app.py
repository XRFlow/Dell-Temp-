...
class AboutDialog(QtWidgets.QDialog):
    def __init__(self, parent: QtWidgets.QWidget | None = None) -> None:
        super().__init__(parent)
        self.setWindowTitle("About DellTemp")
        self.setMinimumWidth(420)
        layout = QtWidgets.QVBoxLayout(self)
        layout.setContentsMargins(20, 18, 20, 16)
        layout.setSpacing(10)

        title = QtWidgets.QLabel(f"DellTemp {__version__}")
        title.setObjectName("AppTitle")
        subtitle = QtWidgets.QLabel("CPU, GPU, and chassis sensor monitor")
...
class SettingsDialog(QtWidgets.QDialog):
    def __init__(
        ...
        super().__init__(parent)
        self.setWindowTitle("DellTemp Settings")
...
class SensorWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, settings: Settings, tray_available: bool = True) -> None:
        ...
        title = QtWidgets.QLabel("DellTemp")
        title.setObjectName("AppTitle")
        subtitle = QtWidgets.QLabel("CPU, GPU, and chassis sensors")
        subtitle.setObjectName("AppSubtitle")
    ...
    def _write_logs(self, readings: list) -> None:
        ...
        try:
            ...
            log_path = logs_dir() / f"delltemp-{datetime.now().strftime('%Y%m%d')}.csv"
    ...
    def _update_window_title(self, readings: list) -> None:
        ...
        self.setWindowTitle(f"DellTemp — {_display_name(reading)} {value}{unit}")
...
class TrayController(QtCore.QObject):
    def __init__(self, window: SensorWindow, settings: Settings) -> None:
        ...
        self._tray.setToolTip("DellTemp")
...
class DellTempApp(QtWidgets.QApplication):
    def __init__(self, argv: list[str], start_minimized: bool) -> None:
        super().__init__(argv)
        self.setApplicationName("DellTemp")
        self.setApplicationDisplayName("DellTemp")
        self.setDesktopFileName("delltemp")
        apply_theme(self)
        self.setWindowIcon(app_icon())
...
def _autostart_command() -> str:
    ...
    return (
        ...
        f"{_desktop_quote(sys.executable)} -m delltemp"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="DellTemp Sensor Dashboard")
    parser.add_argument("--minimized", action="store_true", help="Start minimized")
    args = parser.parse_args()

    app = DellTempApp(sys.argv, start_minimized=args.minimized)
    return app.exec()
